...
namespace PVG.Infrastucture.Domain
{
    ...
    {
        private readonly PVGDbContext _dbContext;
        private readonly IUnitOfWork _unitOfWork;

        public RepositoryBase(PVGDbContext dbContext,
        ...
        public async Task<T> UpdateValueAsync(T entity, object updateRequest)
        {
            if (entity == null) return null;

            _dbContext.Entry(entity).CurrentValues.SetValues(updateRequest);
            return entity;
        }

        public async Task<List<T>> UpdateValuesAsync(List<T> entities, List<object> updateRequests)
        {
            if (entities == null || updateRequests == null) return null;
        ...
        public async Task UpdateAddListAsync(IEnumerable<T> entities)
        {
            await _dbContext.Set<T>().AddRangeAsync(entities);
            await SaveChangesAsync();
        }
